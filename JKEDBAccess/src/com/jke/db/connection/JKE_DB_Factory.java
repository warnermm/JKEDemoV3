...
public class JKE_DB_Factory {
	...
	protected JKE_DB_I createJKE_DB() {
		...
		{
			System.err.println("[Money that Matters] Defaulting database storage to Embedded Derby database.");
		...
		// read DBMS property 
		String dbms = props.getProperty("jdbc.dbms");
		if (dbms != null && "mysql".equalsIgnoreCase(dbms.trim())) {
			return new JKE_DB_MySQL(props);
		} 
		// default is derby
		return new JKE_DB_Derby(props);
	}
}