...
public class AccountLogic {
	...
	/**
	 * Creates a new user account if the account number does not exist. If the
	 * account exists is returns false.
	 * 
	 * @param accountBean
	 */
	public boolean addUserAccount(AccountBean accountBean) {
		if (accountBean == null) return false;
		
		try {
			if (getAccount(accountBean.getAccountNumber()) == null) {
				// we can insert
				loader.pushAccountBean(accountBean);
				return true;
			}
		} catch (SQLException e) {
			e.printStackTrace();
